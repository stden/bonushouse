...
class ActiveAuctionsManager(models.Manager):
    ...
    def get_query_set(self):
        cur_time = now()
        return super(ActiveAuctionsManager, self).get_query_set().filter(is_published=True, is_completed=False, start_date__lte=cur_time, end_date__gte=cur_time)
...
class Auction(ModelWithSeo):
    ...
    completed_date = models.DateTimeField(editable=False, verbose_name='Дата фактического окончания', blank=True, null=True)
    abonements_additional_info = models.ForeignKey(AbonementsAdditionalInfo, blank=True, null=True, editable=False)
    additional_services_additional_info = models.ForeignKey(AdditionalServicesInfo, blank=True, null=True, editable=False)
    ...
    agreement_id = models.CharField(max_length=255, verbose_name='Номер договора', blank=True, null=True, editable=False)
    #Менеджеры
    ...
    def complete(self):
        ...
        if winner_bid:
            ...
            notification_context = Context({'LINK':settings.BASE_URL+str(reverse_lazy('cabinet_auctions')), 'AUCTION':self})
    ...
    def gen_barcode(self):
        bar_code = createBarcodeDrawing('Standard39', value=self.get_agreement_id(), checksum=False, humanReadable=True, barHeight=10*mm, width=400, height=200, isoScale=1)
    ...
    def notify_fitnesshouse(self):
        ...
        if self.is_abonement():
            ...
            other_info['edate'] = timezone.localtime((self.completed_date + datetime.timedelta(days=self.abonements_term+1))).strftime('%Y.%m.%d')
            ...
            other_info['sdate'] = timezone.localtime((self.completed_date+timedelta(days=1))).strftime('%Y.%m.%d')
            other_info['edate'] = timezone.localtime(((self.completed_date+timedelta(days=1)) + datetime.timedelta(days=self.additional_services_term))).strftime('%Y.%m.%d')
            ...
            other_info['pserial'] = ''
            other_info['pnumber'] = ''
            other_info['bd'] = timezone.localtime(self.additional_services_additional_info.birth_date).strftime('%Y.%m.%d')
            other_info['sex'] = ''
            other_info['adv'] = ''
